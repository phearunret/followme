...
class District extends Gps_Controller
{
    ...
    public function prvin()
    {
        ...
        if (count($query)) {
            ...
            foreach ($query as $row) {
                ...
                $str .= '<td>' . anchor('gps-syn/maps/district/edit/' . $row->distr_id, '<i class="fa fa-pencil-square-o" aria-hidden="true"></i>') . '</td>';
                echo $str .= '</tr>';
    ...
    public function track($id = null)
    {
        ...
        $marker = array();
        $coordinate = $this->addr->rquery($id, 'tu_district', 'distr_id');
        $marker['position'] = (is_null($coordinate->distr_nu_latitude) ? '11.562108' : $coordinate->distr_nu_latitude) . ',' . (is_null($coordinate->distr_nu_longitude) ? '104.888535' : $coordinate->distr_nu_longitude);
